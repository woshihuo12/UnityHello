...
{
    public class UICustomGridItem : MonoBehaviour
    {
    ...
    {
        //==========LoopScrollRect==========
        private string mCellPrefabName;
        public string CellPrefabName
        {
            get
            {
                if (string.IsNullOrEmpty(mCellPrefabName))
                {
                    mCellPrefabName = mCellPerfabGo.name;
                }
                return mCellPrefabName;
            }
            set
            {
                mCellPrefabName = value;
            }
        }

        //negative means INFINITE mode
        ...
        private bool mInitInStart = true;
        ...
        public bool ReverseDirection
        {
            ...
            set
            {
                mReverseDirection = value;
            }
        }

        protected int mItemTypeStart = 0;
        protected int mItemTypeEnd = 0;

        protected virtual float GetSize(RectTransform item)
        {
            return 0f;
        }
        ...
        protected virtual Vector2 GetVector(float value)
        {
            return Vector2.zero;
        }

        protected int mDirectionSign = 0;
        ...
        private SimplePM.SpawnPool mSpawnPool;

        public GameObject mCellPerfabGo;
        ...
        public void Init(GameObject cellGo)
        {
            mCellPerfabGo = cellGo;
            InitPerfabPool();
            if (mInitInStart)
            {
                RefillCells();
        ...
        {
            if (Application.isPlaying && mSpawnPool != null)
            {
                mItemTypeStart = 0;
                mItemTypeEnd = 0;
                TotalCount = 0;

                mSpawnPool.DespawnAll();
        ...
        public void RefillCells(int startIdx = 0)
        {
            ...
            {
                StopMovement();
                mItemTypeStart = ReverseDirection ? TotalCount - startIdx : startIdx;
                mItemTypeEnd = mItemTypeStart;
                ...
                {
                    if (TotalCount >= 0 && mItemTypeEnd >= TotalCount)
                    {
                        if (mSpawnPool != null)
                        {
                            mSpawnPool.Despawn(content.GetChild(i), gameObject.transform);
                            --i;
                        }
                    }
                    else
        ...
        {
            if (TotalCount >= 0 && mItemTypeStart - ContentConstraintCount < 0)
            {
                return 0;
            }
            ...
            {
                mItemTypeStart--;
                RectTransform newItem = InstantiateNextItem(mItemTypeStart);
        ...
        {
            if ((TotalCount >= 0 && mItemTypeEnd >= TotalCount - 1) || content.childCount == 0)
            {
                return 0;
            }
        ...
        protected float NewItemAtEnd()
        {
            ...
            for (int i = 0; i < ContentConstraintCount; i++)
            {
                RectTransform newItem = InstantiateNextItem(mItemTypeEnd);
                size = Mathf.Max(GetSize(newItem), size);
                mItemTypeEnd++;
                if (TotalCount >= 0 && mItemTypeEnd >= TotalCount)
                {
        ...
        {
            if ((TotalCount >= 0 && mItemTypeStart < ContentConstraintCount) || content.childCount == 0)
            {
                return 0;
            }
