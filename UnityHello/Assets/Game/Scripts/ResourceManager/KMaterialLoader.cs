...
namespace KEngine
{
    ...
    public class MaterialLoader : AbstractResourceLoader
    {
        ...
        {
            base.Init(url, args);
            AppFacade.Instance.GetManager<KResourceManager>().StartCoroutine(CoLoadSerializeMaterial());
        }
        ...
        private IEnumerator CoLoadSerializeMaterial()
        {
            ...
            yield return AppFacade.Instance.GetManager<KResourceManager>().StartCoroutine(CoGenerateMaterial(Url, sMat));
            Debuger.Assert(Mat);
        ...
        private string ParseMaterialStr(string materialTextureStr, out Vector2 tiling, out Vector2 offset)
        {
            ...
            {
                tiling = new Vector2(float.Parse(textureArr[1]), float.Parse(textureArr[2]));
                offset = new Vector2(float.Parse(textureArr[3]), float.Parse(textureArr[4]));
            }
