    ...
    public class KHttpDownloader : MonoBehaviour, IDisposable
    {
            ...
            Action successCallback)
        {
            ...
            if (_useContinue && System.IO.File.Exists(TmpDownloadPath))
            {
                ...
                Log.Info("Resume.... from {0}", lStartPos);
            ...
            try
            {
                ...
                Log.Info("Getting Response : {0}", url);
                ...
                using (var response = request.GetResponse()) // TODO: Async Timeout
                {
                    ...
                    Log.Info("Getted Response : {0}", url);
                    if (IsFinished)
                    {
            ...
            catch (Exception ex)
            {
                ...
                Log.Error("下载过程中出现错误:" + ex.ToString());
        ...
        private void OnDestroy()
        {
            if (!FinishedFlag)
            {
                ...
                Log.Error(string.Format("[HttpDownloader]Not finish but destroy: {0}", Url));
            }
        }
